---
name: release:major-version-brew
description: Bump major version (e.g. 1.0.1 → 2.0.0), build, release to GitHub, and update Homebrew formula.
---

# Release Major Version with Homebrew Update

Perform a **major** version bump and full release pipeline.

## Steps

1. **Read current version** from `src/config.ts` (the `VERSION` constant) and `package.json`.

2. **Compute new version**: Increment the major segment, reset minor and patch to 0.
   - Example: `1.0.1` → `2.0.0`
