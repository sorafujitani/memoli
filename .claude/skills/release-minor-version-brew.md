---
name: release:minor-version-brew
description: Bump minor version (e.g. 1.0.1 → 1.1.0), build, release to GitHub, and update Homebrew formula.
---

# Release Minor Version with Homebrew Update

Perform a **minor** version bump and full release pipeline.

## Steps

1. **Read current version** from `src/config.ts` (the `VERSION` constant) and `package.json`.

2. **Compute new version**: Increment the minor segment, reset patch to 0.
   - Example: `1.0.1` → `1.1.0`
