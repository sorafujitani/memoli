...
| `task_get`    | キーワードでタスクを検索 | `query`        | —                                                        |
| `task_update` | タスクを更新             | `query`        | `title`, `status`, `priority`, `tags`, `dueDate`, `memo` |
| `task_remove` | タスクを削除             | `query`        | —                                                        |
...
タスクをキーワードで検索して 1 件取得します。タイトルの部分一致（大文字小文字無視）または ID の前方一致で検索できます。

```
入力:
  query: string (必須)       — タスクのタイトル(部分一致) または ID

出力: Task オブジェクト (JSON)
```

例: `query: "README"` → タイトル「READMEを書く」にマッチ

#### `task_update`

タスクをキーワードで検索して更新します。`status` を指定するとステータスを変更し、それ以外のフィールドを指定するとプロパティを更新します。
...
例: `query: "README", status: "done"` → READMEタスクを完了にする
...
| メッセージ                         | 原因                                       |
| ---------------------------------- | ------------------------------------------ |
| `Task not found: <query>`          | 指定したキーワードにマッチするタスクがない |
